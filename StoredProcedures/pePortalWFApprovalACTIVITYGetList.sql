if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalACTIVITYGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalACTIVITYGetList 
GO
...
-- Returns a query resultset from table [dbo].[ACTIVITY]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalACTIVITYGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY ACTIVITY_.[USERID],ACTIVITY_.[CMPNYNAM] asc '
        ...
        SET @l_query_select = 'WITH ACTIVITY_ AS ( SELECT  '
        ...
        SET @l_query_cols = 
            ...
            ACTIVITY_.[LOGINTIM],
            ACTIVITY_.[SQLSESID],
            ACTIVITY_.[Language_ID],
            ACTIVITY_.[IsWebClient],
            ACTIVITY_.[IsOffline],
            ACTIVITY_.[DEX_ROW_ID],
            CAST(BINARY_CHECKSUM(ACTIVITY_.[USERID],ACTIVITY_.[CMPNYNAM],ACTIVITY_.[LOGINDAT],ACTIVITY_.[LOGINTIM],ACTIVITY_.[SQLSESID],ACTIVITY_.[Language_ID],ACTIVITY_.[IsWebClient],ACTIVITY_.[IsOffline],ACTIVITY_.[DEX_ROW_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM ACTIVITY_ '
