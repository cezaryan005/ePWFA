if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_Approver_Pending_Tasks2GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_Approver_Pending_Tasks2GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_Approver_Pending_Tasks]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_Approver_Pending_Tasks2GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_Approver_Pending_Tasks_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_Approver_Pending_Tasks_.[Doc_Type],
            sel_Approver_Pending_Tasks_.[Company_Desc],
            sel_Approver_Pending_Tasks_.[Doc_No],
            sel_Approver_Pending_Tasks_.[Date_Assigned],
            sel_Approver_Pending_Tasks_.[W_U_ID],
            sel_Approver_Pending_Tasks_.[PK_ID],
            sel_Approver_Pending_Tasks_.[C_ID],
            sel_Approver_Pending_Tasks_.[Doc_Title],
            sel_Approver_Pending_Tasks_.[Doc_Total],
            sel_Approver_Pending_Tasks_.[SSC_CompanyID],
            sel_Approver_Pending_Tasks_.[SortOrder]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_Approver_Pending_Tasks_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            sel_Approver_Pending_Tasks_.[SortOrder]'
        SET @l_query_from = 
            ' FROM [dbo].[sel_Approver_Pending_Tasks] sel_Approver_Pending_Tasks_ ' + 
            'WHERE 1=2;'
