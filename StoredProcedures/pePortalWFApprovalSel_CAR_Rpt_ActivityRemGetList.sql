if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_CAR_Rpt_ActivityRemGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_CAR_Rpt_ActivityRemGetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_CAR_Rpt_ActivityRem]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_CAR_Rpt_ActivityRemGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_CAR_Rpt_ActivityRem] sel_CAR_Rpt_ActivityRem_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY sel_CAR_Rpt_ActivityRem_.[WCA_ID] asc '
        ...
        SET @l_query_select = 'WITH sel_CAR_Rpt_ActivityRem_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_CAR_Rpt_ActivityRem_.[WCD_No],
            sel_CAR_Rpt_ActivityRem_.[WCD_C_ID],
            sel_CAR_Rpt_ActivityRem_.[WCA_Remark],
            sel_CAR_Rpt_ActivityRem_.[WCA_ID],
            sel_CAR_Rpt_ActivityRem_.[WCA_Status],
            CAST(BINARY_CHECKSUM(sel_CAR_Rpt_ActivityRem_.[WCD_No],sel_CAR_Rpt_ActivityRem_.[WCD_C_ID],sel_CAR_Rpt_ActivityRem_.[WCA_Remark],sel_CAR_Rpt_ActivityRem_.[WCA_ID],sel_CAR_Rpt_ActivityRem_.[WCA_Status]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_CAR_Rpt_ActivityRem_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(sel_CAR_Rpt_ActivityRem_.[WCD_No],sel_CAR_Rpt_ActivityRem_.[WCD_C_ID],sel_CAR_Rpt_ActivityRem_.[WCA_Remark],sel_CAR_Rpt_ActivityRem_.[WCA_ID],sel_CAR_Rpt_ActivityRem_.[WCA_Status]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[sel_CAR_Rpt_ActivityRem] sel_CAR_Rpt_ActivityRem_ ' + 
            'WHERE 1=2;'
