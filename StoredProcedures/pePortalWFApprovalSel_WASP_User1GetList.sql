if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WASP_User1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WASP_User1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WASP_User]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WASP_User1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_WASP_User] sel_WASP_User_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY sel_WASP_User_.[W_U_ID] asc '
        ...
        SET @l_query_select = 'WITH sel_WASP_User_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WASP_User_.[W_U_ID],
            sel_WASP_User_.[W_U_User_Name],
            sel_WASP_User_.[W_U_Password],
            sel_WASP_User_.[W_U_Full_Name],
            sel_WASP_User_.[W_U_Designation],
            sel_WASP_User_.[W_U_GP_User_Name],
            CAST(BINARY_CHECKSUM(sel_WASP_User_.[W_U_ID],sel_WASP_User_.[W_U_User_Name],sel_WASP_User_.[W_U_Password],sel_WASP_User_.[W_U_Full_Name],sel_WASP_User_.[W_U_Designation],sel_WASP_User_.[W_U_GP_User_Name]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_WASP_User_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(sel_WASP_User_.[W_U_ID],sel_WASP_User_.[W_U_User_Name],sel_WASP_User_.[W_U_Password],sel_WASP_User_.[W_U_Full_Name],sel_WASP_User_.[W_U_Designation],sel_WASP_User_.[W_U_GP_User_Name]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[sel_WASP_User] sel_WASP_User_ ' + 
            'WHERE 1=2;'
