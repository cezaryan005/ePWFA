if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WASP_WF_Creator1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WASP_WF_Creator1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WASP_WF_Creator]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WASP_WF_Creator1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_WASP_WF_Creator] sel_WASP_WF_Creator_'
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_WASP_WF_Creator_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WASP_WF_Creator_.[W_U_ID],
            sel_WASP_WF_Creator_.[W_U_User_Name],
            sel_WASP_WF_Creator_.[W_U_Full_Name],
            sel_WASP_WF_Creator_.[W_R_ID],
            sel_WASP_WF_Creator_.[W_R_Name]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_WASP_WF_Creator_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            sel_WASP_WF_Creator_.[W_R_Name]'
        SET @l_query_from = 
            ' FROM [dbo].[sel_WASP_WF_Creator] sel_WASP_WF_Creator_ ' + 
            'WHERE 1=2;'
