if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WCAR_Activity_WCAR_Doc1Update') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WCAR_Activity_WCAR_Doc1Update 
GO
...
CREATE PROCEDURE pePortalWFApprovalSel_WCAR_Activity_WCAR_Doc1Update
    @p_WCA_ID int,
    @pk_WCA_ID int,
    @p_WCA_WS_ID int,
    @p_WCA_WSD_ID int,
...
-- Check whether the record still exists before doing update
    ...
    ELSE
        BEGIN
            ...
            Select @l_newValue = CAST(BINARY_CHECKSUM([WCA_ID],[WCA_WS_ID],[WCA_WSD_ID],[WCA_WDT_ID],[WCA_W_U_ID],[WCA_WCD_ID],[WCA_Status],[WCA_Date_Assign],[WCA_Date_Action],[WCA_Remark],[WCD_No],[WCD_Project_Title],[WCD_Exp_Total],[WCA_Is_Done],[WCD_U_ID],[WCD_Remark],[WCD_WCur_ID],[WCD_Request_Date],[WCD_C_ID]) AS nvarchar(max)) 
