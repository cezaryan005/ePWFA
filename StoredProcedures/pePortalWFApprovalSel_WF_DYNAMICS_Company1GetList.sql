if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WF_DYNAMICS_Company1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WF_DYNAMICS_Company1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WF_DYNAMICS_Company]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WF_DYNAMICS_Company1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_WF_DYNAMICS_Company] sel_WF_DYNAMICS_Company_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY sel_WF_DYNAMICS_Company_.[Company_ID] asc '
        ...
        SET @l_query_select = 'WITH sel_WF_DYNAMICS_Company_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WF_DYNAMICS_Company_.[Company_ID],
            sel_WF_DYNAMICS_Company_.[Company_Desc],
            sel_WF_DYNAMICS_Company_.[Company_Short_Name],
            sel_WF_DYNAMICS_Company_.[INTERID],
            sel_WF_DYNAMICS_Company_.[FULLADDRESS],
            CAST(BINARY_CHECKSUM(sel_WF_DYNAMICS_Company_.[Company_ID],sel_WF_DYNAMICS_Company_.[Company_Desc],sel_WF_DYNAMICS_Company_.[Company_Short_Name],sel_WF_DYNAMICS_Company_.[INTERID],sel_WF_DYNAMICS_Company_.[FULLADDRESS]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_WF_DYNAMICS_Company_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(sel_WF_DYNAMICS_Company_.[Company_ID],sel_WF_DYNAMICS_Company_.[Company_Desc],sel_WF_DYNAMICS_Company_.[Company_Short_Name],sel_WF_DYNAMICS_Company_.[INTERID],sel_WF_DYNAMICS_Company_.[FULLADDRESS]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[sel_WF_DYNAMICS_Company] sel_WF_DYNAMICS_Company_ ' + 
            'WHERE 1=2;'
