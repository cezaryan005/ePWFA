if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WF_Table_Fields1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WF_Table_Fields1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WF_Table_Fields]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WF_Table_Fields1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_WF_Table_Fields] sel_WF_Table_Fields_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY sel_WF_Table_Fields_.[Row] asc '
        ...
        SET @l_query_select = 'WITH sel_WF_Table_Fields_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WF_Table_Fields_.[Row],
            sel_WF_Table_Fields_.[TABLE_NAME],
            sel_WF_Table_Fields_.[COLUMN_NAME],
            sel_WF_Table_Fields_.[IS_NULLABLE],
            sel_WF_Table_Fields_.[DATA_TYPE],
            CAST(BINARY_CHECKSUM(sel_WF_Table_Fields_.[Row],sel_WF_Table_Fields_.[TABLE_NAME],sel_WF_Table_Fields_.[COLUMN_NAME],sel_WF_Table_Fields_.[IS_NULLABLE],sel_WF_Table_Fields_.[DATA_TYPE]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_WF_Table_Fields_ '
