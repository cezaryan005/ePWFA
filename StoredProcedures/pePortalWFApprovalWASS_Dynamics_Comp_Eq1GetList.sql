if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWASS_Dynamics_Comp_Eq1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWASS_Dynamics_Comp_Eq1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WASS_Dynamics_Comp_Eq]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWASS_Dynamics_Comp_Eq1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WASS_Dynamics_Comp_Eq] WASS_Dynamics_Comp_Eq_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WASS_Dynamics_Comp_Eq_.[WDCE_ID] asc '
        ...
        SET @l_query_select = 'WITH WASS_Dynamics_Comp_Eq_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WASS_Dynamics_Comp_Eq_.[WDCE_ID],
            WASS_Dynamics_Comp_Eq_.[WDCE_WASS_CompID],
            WASS_Dynamics_Comp_Eq_.[WDCE_Dynamics_CompID],
            WASS_Dynamics_Comp_Eq_.[WDCE_CompName],
            CAST(BINARY_CHECKSUM(WASS_Dynamics_Comp_Eq_.[WDCE_ID],WASS_Dynamics_Comp_Eq_.[WDCE_WASS_CompID],WASS_Dynamics_Comp_Eq_.[WDCE_Dynamics_CompID],WASS_Dynamics_Comp_Eq_.[WDCE_CompName]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WASS_Dynamics_Comp_Eq_ '
