if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWAttach_Type1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWAttach_Type1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WAttach_Type]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWAttach_Type1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WAttach_Type] WAttach_Type_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WAttach_Type_.[WAT_ID] asc '
        ...
        SET @l_query_select = 'WITH WAttach_Type_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WAttach_Type_.[WAT_ID],
            WAttach_Type_.[WAT_Name],
            WAttach_Type_.[WAT_Operation],
            WAttach_Type_.[WAT_Order],
            CAST(BINARY_CHECKSUM(WAttach_Type_.[WAT_ID],WAttach_Type_.[WAT_Name],WAttach_Type_.[WAT_Operation],WAttach_Type_.[WAT_Order]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WAttach_Type_ '
