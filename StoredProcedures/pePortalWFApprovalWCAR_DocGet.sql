if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCAR_DocGet') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCAR_DocGet 
GO
...
CREATE PROCEDURE pePortalWFApprovalWCAR_DocGet
        @pk_WCD_ID int
AS
DECLARE
    @l_count int
