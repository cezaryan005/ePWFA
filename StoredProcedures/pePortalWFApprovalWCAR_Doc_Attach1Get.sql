if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCAR_Doc_Attach1Get') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCAR_Doc_Attach1Get 
GO
...
CREATE PROCEDURE pePortalWFApprovalWCAR_Doc_Attach1Get
        @pk_WCDA_ID int
AS
DECLARE
    @l_count int
