if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCAR_Doc_Attach1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCAR_Doc_Attach1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WCAR_Doc_Attach]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWCAR_Doc_Attach1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WCAR_Doc_Attach] WCAR_Doc_Attach_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WCAR_Doc_Attach_.[WCDA_ID] asc '
        ...
        SET @l_query_select = 'WITH WCAR_Doc_Attach_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WCAR_Doc_Attach_.[WCDA_ID],
            WCAR_Doc_Attach_.[WCDA_WCD_ID],
            WCAR_Doc_Attach_.[WCDA_Desc],
            WCAR_Doc_Attach_.[WCDA_File],
            WCAR_Doc_Attach_.[WCDA_WAT_ID],
            CAST(BINARY_CHECKSUM(WCAR_Doc_Attach_.[WCDA_ID],WCAR_Doc_Attach_.[WCDA_WCD_ID],WCAR_Doc_Attach_.[WCDA_Desc],WCAR_Doc_Attach_.[WCDA_File],WCAR_Doc_Attach_.[WCDA_WAT_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WCAR_Doc_Attach_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WCAR_Doc_Attach_.[WCDA_ID],WCAR_Doc_Attach_.[WCDA_WCD_ID],WCAR_Doc_Attach_.[WCDA_Desc],WCAR_Doc_Attach_.[WCDA_File],WCAR_Doc_Attach_.[WCDA_WAT_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WCAR_Doc_Attach] WCAR_Doc_Attach_ ' + 
            'WHERE 1=2;'
