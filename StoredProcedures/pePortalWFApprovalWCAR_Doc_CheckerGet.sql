if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCAR_Doc_CheckerGet') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCAR_Doc_CheckerGet 
GO
...
CREATE PROCEDURE pePortalWFApprovalWCAR_Doc_CheckerGet
        @pk_WCDC_ID int
AS
DECLARE
    @l_count int
