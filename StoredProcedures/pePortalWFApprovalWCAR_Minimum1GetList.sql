if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCAR_Minimum1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCAR_Minimum1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WCAR_Minimum]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWCAR_Minimum1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WCAR_Minimum] WCAR_Minimum_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WCAR_Minimum_.[WCM_ID] asc '
        ...
        SET @l_query_select = 'WITH WCAR_Minimum_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WCAR_Minimum_.[WCM_ID],
            WCAR_Minimum_.[WCM_C_ID],
            WCAR_Minimum_.[WCM_Min],
            WCAR_Minimum_.[WCM_WCur_ID],
            CAST(BINARY_CHECKSUM(WCAR_Minimum_.[WCM_ID],WCAR_Minimum_.[WCM_C_ID],WCAR_Minimum_.[WCM_Min],WCAR_Minimum_.[WCM_WCur_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WCAR_Minimum_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WCAR_Minimum_.[WCM_ID],WCAR_Minimum_.[WCM_C_ID],WCAR_Minimum_.[WCM_Min],WCAR_Minimum_.[WCM_WCur_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WCAR_Minimum] WCAR_Minimum_ ' + 
            'WHERE 1=2;'
