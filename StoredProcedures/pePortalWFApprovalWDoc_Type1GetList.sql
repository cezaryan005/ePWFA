if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWDoc_Type1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWDoc_Type1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WDoc_Type]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWDoc_Type1GetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WDoc_Type_.[WDT_ID] asc '
        ...
        SET @l_query_select = 'WITH WDoc_Type_ AS ( SELECT  '
        ...
        SET @l_query_cols = 
            ...
            WDoc_Type_.[WDT_Desc],
            WDoc_Type_.[WDT_Type],
            WDoc_Type_.[WDT_Limit],
            WDoc_Type_.[WDT_Minimum],
            CAST(BINARY_CHECKSUM(WDoc_Type_.[WDT_ID],WDoc_Type_.[WDT_C_ID],WDoc_Type_.[WDT_Name],WDoc_Type_.[WDT_Desc],WDoc_Type_.[WDT_Type],WDoc_Type_.[WDT_Limit],WDoc_Type_.[WDT_Minimum]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WDoc_Type_ '
