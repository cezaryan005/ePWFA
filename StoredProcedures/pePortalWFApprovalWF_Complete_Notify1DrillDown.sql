if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWF_Complete_Notify1DrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWF_Complete_Notify1DrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalWF_Complete_Notify1DrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WF_Complete_Notify'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
