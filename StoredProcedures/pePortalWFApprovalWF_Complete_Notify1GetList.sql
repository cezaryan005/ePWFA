if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWF_Complete_Notify1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWF_Complete_Notify1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WF_Complete_Notify]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWF_Complete_Notify1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WF_Complete_Notify] WF_Complete_Notify_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WF_Complete_Notify_.[WFCN_ID] asc '
        ...
        SET @l_query_select = 'WITH WF_Complete_Notify_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WF_Complete_Notify_.[WFCN_ID],
            WF_Complete_Notify_.[WFCN_C_ID],
            WF_Complete_Notify_.[WFCN_U_ID],
            WF_Complete_Notify_.[WFCN_DocType],
            CAST(BINARY_CHECKSUM(WF_Complete_Notify_.[WFCN_ID],WF_Complete_Notify_.[WFCN_C_ID],WF_Complete_Notify_.[WFCN_U_ID],WF_Complete_Notify_.[WFCN_DocType]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WF_Complete_Notify_ '
