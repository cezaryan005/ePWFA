if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPO_Doc_Attach1GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPO_Doc_Attach1GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalWPO_Doc_Attach1GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WPO_Doc_Attach'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[WPO_Doc_Attach] WPO_Doc_Attach_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
