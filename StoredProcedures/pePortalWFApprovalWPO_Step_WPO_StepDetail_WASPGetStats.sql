if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPO_Step_WPO_StepDetail_WASPGetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPO_Step_WPO_StepDetail_WASPGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalWPO_Step_WPO_StepDetail_WASPGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WPO_Step_WPO_StepDetail_WASP'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[WPO_Step_WPO_StepDetail_WASP] WPO_Step_WPO_StepDetail_WASP_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
