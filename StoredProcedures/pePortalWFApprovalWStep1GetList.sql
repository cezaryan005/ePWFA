if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWStep1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWStep1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WStep]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWStep1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WStep] WStep_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WStep_.[WS_ID] asc '
        ...
        SET @l_query_select = 'WITH WStep_ AS ( SELECT  '
        ...
        SET @l_query_cols = 
            ...
            WStep_.[WS_Step_Type],
            WStep_.[WS_Desc],
            WStep_.[WS_Approval_Needed],
            WStep_.[WS_C_ID],
            CAST(BINARY_CHECKSUM(WStep_.[WS_ID],WStep_.[WS_WDT_ID],WStep_.[WS_ID_Next],WStep_.[WS_Step_Type],WStep_.[WS_Desc],WStep_.[WS_Approval_Needed],WStep_.[WS_C_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WStep_ '
