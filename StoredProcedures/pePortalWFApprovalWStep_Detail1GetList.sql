if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWStep_Detail1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWStep_Detail1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WStep_Detail]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWStep_Detail1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WStep_Detail] WStep_Detail_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WStep_Detail_.[WSD_ID] asc '
        ...
        SET @l_query_select = 'WITH WStep_Detail_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WStep_Detail_.[WSD_ID],
            WStep_Detail_.[WSD_WS_ID],
            WStep_Detail_.[WSD_W_U_ID],
            WStep_Detail_.[WSD_Desc],
            WStep_Detail_.[WSD_Status],
            WStep_Detail_.[WSD_Variable_Ref],
            WStep_Detail_.[WSD_Variable_SQL],
            WStep_Detail_.[WSD_Expire],
            WStep_Detail_.[WSD_W_U_ID_Delegate],
            WStep_Detail_.[WSD_Is_Escalate],
            CAST(BINARY_CHECKSUM(WStep_Detail_.[WSD_ID],WStep_Detail_.[WSD_WS_ID],WStep_Detail_.[WSD_W_U_ID],WStep_Detail_.[WSD_Desc],WStep_Detail_.[WSD_Status],WStep_Detail_.[WSD_Variable_Ref],WStep_Detail_.[WSD_Variable_SQL],WStep_Detail_.[WSD_Expire],WStep_Detail_.[WSD_W_U_ID_Delegate],WStep_Detail_.[WSD_Is_Escalate]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WStep_Detail_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WStep_Detail_.[WSD_ID],WStep_Detail_.[WSD_WS_ID],WStep_Detail_.[WSD_W_U_ID],WStep_Detail_.[WSD_Desc],WStep_Detail_.[WSD_Status],WStep_Detail_.[WSD_Variable_Ref],WStep_Detail_.[WSD_Variable_SQL],WStep_Detail_.[WSD_Expire],WStep_Detail_.[WSD_W_U_ID_Delegate],WStep_Detail_.[WSD_Is_Escalate]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WStep_Detail] WStep_Detail_ ' + 
            'WHERE 1=2;'
