if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWTask_Delegation1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWTask_Delegation1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WTask_Delegation]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWTask_Delegation1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WTask_Delegation] WTask_Delegation_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WTask_Delegation_.[WTD_ID] asc '
        ...
        SET @l_query_select = 'WITH WTask_Delegation_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WTask_Delegation_.[WTD_ID],
            WTask_Delegation_.[WTD_W_U_ID],
            WTask_Delegation_.[WTD_W_U_ID_Delegate],
            WTask_Delegation_.[WTD_WDT_Type],
            WTask_Delegation_.[WTD_From],
            WTask_Delegation_.[WTD_To],
            WTask_Delegation_.[WTD_C_ID],
            CAST(BINARY_CHECKSUM(WTask_Delegation_.[WTD_ID],WTask_Delegation_.[WTD_W_U_ID],WTask_Delegation_.[WTD_W_U_ID_Delegate],WTask_Delegation_.[WTD_WDT_Type],WTask_Delegation_.[WTD_From],WTask_Delegation_.[WTD_To],WTask_Delegation_.[WTD_C_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WTask_Delegation_ '
