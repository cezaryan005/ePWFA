if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWTask_DelegationGet') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWTask_DelegationGet 
GO
...
CREATE PROCEDURE pePortalWFApprovalWTask_DelegationGet
        @pk_WTD_ID int
AS
DECLARE
    @l_count int
