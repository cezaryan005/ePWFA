if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalW_Email_Default1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalW_Email_Default1GetList 
GO
...
-- Returns a query resultset from table [dbo].[W_Email_Default]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalW_Email_Default1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[W_Email_Default] W_Email_Default_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY W_Email_Default_.[WED_ID] asc '
        ...
        SET @l_query_select = 'WITH W_Email_Default_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'W_Email_Default_.[WED_ID],
            W_Email_Default_.[WED_Type],
            W_Email_Default_.[WED_Template],
            CAST(BINARY_CHECKSUM(W_Email_Default_.[WED_ID],W_Email_Default_.[WED_Type],W_Email_Default_.[WED_Template]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM W_Email_Default_ '
