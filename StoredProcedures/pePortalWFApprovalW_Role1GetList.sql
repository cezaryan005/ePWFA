if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalW_Role1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalW_Role1GetList 
GO
...
-- Returns a query resultset from table [dbo].[W_Role]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalW_Role1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[W_Role] W_Role_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY W_Role_.[W_R_ID] asc '
        ...
        SET @l_query_select = 'WITH W_Role_ AS ( SELECT  '
        ...
        SET @l_query_cols = 
            ...
            W_Role_.[W_R_Name],
            W_Role_.[W_R_Desc],
            CAST(BINARY_CHECKSUM(W_Role_.[W_R_ID],W_Role_.[W_R_A_ID],W_Role_.[W_R_Name],W_Role_.[W_R_Desc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM W_Role_ '
