if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalW_User1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalW_User1GetList 
GO
...
-- Returns a query resultset from table [dbo].[W_User]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalW_User1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[W_User] W_User_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY W_User_.[W_U_ID] asc '
        ...
        SET @l_query_select = 'WITH W_User_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'W_User_.[W_U_ID],
            W_User_.[W_U_User_Name],
            W_User_.[W_U_Password],
            W_User_.[W_U_Full_Name],
            W_User_.[W_U_Email],
            W_User_.[W_U_GP_User_Name],
            W_User_.[W_U_Designation],
            CAST(BINARY_CHECKSUM(W_User_.[W_U_ID],W_User_.[W_U_User_Name],W_User_.[W_U_Password],W_User_.[W_U_Full_Name],W_User_.[W_U_Email],W_User_.[W_U_GP_User_Name],W_User_.[W_U_Designation]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM W_User_ '
