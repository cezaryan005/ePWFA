if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalW_User_DYNAMICS_Company1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalW_User_DYNAMICS_Company1GetList 
GO
...
-- Returns a query resultset from table [dbo].[W_User_DYNAMICS_Company]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalW_User_DYNAMICS_Company1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[W_User_DYNAMICS_Company] W_User_DYNAMICS_Company_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY W_User_DYNAMICS_Company_.[W_UDC_ID] asc '
        ...
        SET @l_query_select = 'WITH W_User_DYNAMICS_Company_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'W_User_DYNAMICS_Company_.[W_UDC_ID],
            W_User_DYNAMICS_Company_.[W_UDC_U_ID],
            W_User_DYNAMICS_Company_.[W_UDC_C_ID],
            CAST(BINARY_CHECKSUM(W_User_DYNAMICS_Company_.[W_UDC_ID],W_User_DYNAMICS_Company_.[W_UDC_U_ID],W_User_DYNAMICS_Company_.[W_UDC_C_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM W_User_DYNAMICS_Company_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(W_User_DYNAMICS_Company_.[W_UDC_ID],W_User_DYNAMICS_Company_.[W_UDC_U_ID],W_User_DYNAMICS_Company_.[W_UDC_C_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[W_User_DYNAMICS_Company] W_User_DYNAMICS_Company_ ' + 
            'WHERE 1=2;'
