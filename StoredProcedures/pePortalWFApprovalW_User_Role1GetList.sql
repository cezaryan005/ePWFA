if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalW_User_Role1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalW_User_Role1GetList 
GO
...
-- Returns a query resultset from table [dbo].[W_User_Role]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalW_User_Role1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[W_User_Role] W_User_Role_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY W_User_Role_.[W_UR_ID] asc '
        ...
        SET @l_query_select = 'WITH W_User_Role_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'W_User_Role_.[W_UR_ID],
            W_User_Role_.[W_UR_U_ID],
            W_User_Role_.[W_UR_R_ID],
            CAST(BINARY_CHECKSUM(W_User_Role_.[W_UR_ID],W_User_Role_.[W_UR_U_ID],W_User_Role_.[W_UR_R_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM W_User_Role_ '
